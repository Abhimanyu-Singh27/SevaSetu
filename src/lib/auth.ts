...
export async function setSession(user: SessionUser) {
  ...
  await prisma.session.create({ data: { id: sessionId, userId: user.userId, refreshHash: hashSessionId(sessionId), expiresAt: new Date(Date.now() + persistentSessionAge * 1000) } });
