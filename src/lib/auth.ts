...
const persistentSessionExpiry = new Date("9999-12-31T23:59:59.999Z");
const persistentSessionMaxAge = 2_147_483_647;
...
export async function setSession(user: SessionUser) {
  ...
  await prisma.session.create({ data: { id: sessionId, userId: user.userId, refreshHash: hashSessionId(sessionId), expiresAt: persistentSessionExpiry } });
...
export async function getSession(): Promise<SessionUser | null> {
  try {
    ...
    const session = await prisma.session.findFirst({ where: { id: payload.sub, userId: payload.userId, revokedAt: null, expiresAt: { gt: new Date() } }, select: { id: true } });
