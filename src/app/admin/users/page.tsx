...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="state-page"><section className="state-panel" style={{ maxWidth: 1100, textAlign: "left" }}><Link className="back-link" href="/admin">Dashboard</Link><div className="workspace-heading"><div><span className="kicker">Admin workspace</span><h1>Users</h1><p>Search and review customer and worker accounts.</p></div><a className="button" href={exportHref}>Export CSV</a></div><form className="admin-filters" method="get"><input name="q" defaultValue={q} placeholder="Search name or email" /><select name="role" defaultValue={role || ""}><option value="">All roles</option><option value="CUSTOMER">Customers</option><option value="WORKER">Workers</option></select><select name="status" defaultValue={status || ""}><option value="">All statuses</option><option value="ACTIVE">Active</option><option value="BLOCKED">Blocked</option><option value="SUSPENDED">Suspended</option><option value="DEACTIVATED">Deactivated</option></select><button className="button" type="submit">Filter</button></form><div className="workspace-table">{users.map((user) => <div className="workspace-row" key={user.id}><div><strong>{user.displayName || user.customerProfile?.fullName || user.workerProfile?.fullName || user.email}</strong><small>{user.email} · {user.role} · Joined {user.createdAt.toLocaleDateString("en-IN")}</small></div><span className="status-pill">{user.status}</span>{user.workerProfile && <small>{user.workerProfile.verificationStatus}</small>}<Link className="outline-button" href={`/admin/users/${user.id}`}>View details</Link><UserStatusButton userId={user.id} status={user.status} /></div>)}</div><div className="workspace-stats"><span>Page {page} of {Math.max(Math.ceil(total / limit), 1)}</span><span>{total} matching accounts</span>{page > 1 && <Link href={`/admin/users?${new URLSearchParams({ ...Object.fromEntries(query), page: String(page - 1) }).toString()}`}>Previous</Link>}{page * limit < total && <Link href={`/admin/users?${new URLSearchParams({ ...Object.fromEntries(query), page: String(page + 1) }).toString()}`}>Next</Link>}</div></section></main>;
}
