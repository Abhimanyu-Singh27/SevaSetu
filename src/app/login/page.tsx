...
export default function LoginPage() {
  ...
  return <main className="auth-page"><a className="brand auth-brand" href="/"><SevaSetuLogo /></a><section className="auth-card"><div className="auth-intro"><span className="portal-icon"><ShieldCheck size={19} /></span><span className="kicker">Welcome back</span><h1>Let&apos;s get things done.</h1><p>Sign in to manage your services, requests, and conversations.</p></div><div className="role-switch" role="tablist" aria-label="Account type"><button className={role === "CUSTOMER" ? "active" : ""} onClick={() => setRole("CUSTOMER")} type="button">I need a service</button><button className={role === "WORKER" ? "active" : ""} onClick={() => setRole("WORKER")} type="button">I provide services</button></div><form onSubmit={submit}><label>Email address<input type="email" required autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label>Password<input type="password" required autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>{error && <p className="auth-error" role="alert">{error}</p>}<button className="button auth-submit" disabled={loading}>{loading ? <LoaderCircle className="spin" size={17} /> : <>Sign in <ArrowRight size={17} /></>}</button></form><p className="auth-foot"><a href="/forgot-password">Forgot password?</a></p><p className="auth-foot">New to SevaSetu? <a href="/register">Create an account</a> · <a href="/admin/setup">Administrator setup</a></p></section></main>;
}
