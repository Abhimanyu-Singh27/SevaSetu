...
export default function RegisterPage() {
  ...
  return <main className="auth-page"><Link className="brand auth-brand" href="/"><SevaSetuLogo /></Link><section className="auth-card"><div className="auth-intro"><span className="kicker">Join the network</span><h1>Make everyday work easier.</h1><p>Create your SevaSetu account and verify your email to get started.</p></div><div className="role-switch" role="tablist" aria-label="Account type"><button role="tab" aria-selected={form.role === "CUSTOMER"} className={form.role === "CUSTOMER" ? "active" : ""} onClick={() => setForm({ ...form, role: "CUSTOMER" })} type="button">I need a service</button><button role="tab" aria-selected={form.role === "WORKER"} className={form.role === "WORKER" ? "active" : ""} onClick={() => setForm({ ...form, role: "WORKER" })} type="button">I provide services</button></div><form onSubmit={submit}><label>Full name<input required value={form.fullName} onChange={(event) => setForm({ ...form, fullName: event.target.value })} /></label><label>Email address<input type="email" required autoComplete="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label><label>Password<input type="password" required minLength={10} autoComplete="new-password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} /></label>{error && <p className="auth-error" role="alert">{error}</p>}<button className="button auth-submit" disabled={loading}>{loading ? <LoaderCircle className="spin" size={17} /> : <>Create account <ArrowRight size={17} /></>}</button></form><p className="auth-foot">Already registered? <Link href="/login">Sign in</Link></p></section></main>;
}
