...
import { getObjectMetadata, scanObject } from "@/lib/storage";
...
export async function POST(request: Request) {
  ...
  const limit = await rateLimit(`uploads:complete:${session.userId}`, 30, 3600);
  ...
  try {
    ...
    if (!metadata || metadata.sizeBytes > maxBytes || metadata.sizeBytes !== sizeBytes || metadata.contentType !== contentType || !(await scanObject(objectKey))) return NextResponse.json({ error: "Upload could not be verified" }, { status: 400 });
